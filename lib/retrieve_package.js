...
function checkLibraryExists(options, callback) {
  console.log('check if this exists', options.dst);
  console.log('exists in fxn?', fs.existsSync(options.dst));
  existsAsync(options.dst, function(err, found) {
        ...
        if (!exists) {
          ...
          if (libraries.length - 1 === i) {
            q.awaitAll(function(err) {
              if (err) return callback(err);
              return callback(null, newLibraries);
            });
          }
        }
      });
